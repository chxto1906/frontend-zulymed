<template>
    <v-row>
        ...
        <v-col cols="12" class="py-0">
            <v-data-table
                no-data-text="Empresas no disponibles"
            ...
            >
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <router-link :to="{ name: 'EmpresaDetalle', params: { id: item._id, action:'editar'}}" >
                        <v-icon
                            small
                            class="mr-2"
    ...
    methods: {
        ...
        async readDataFromAPI() {
            ...
            if (userLogged) {
                ...
                let response = await api.call_respuesta_directa('/empresas') 
                empresas = response.data ? response.data.data : [];
                this.loading = false;
                this.totalEmpresas = response.data.totalEmpresas;
        ...
        async deleteApi() {
            ...
            if (userLogged) {
                ...
                response = await api.call_respuesta_directa(`/empresas/${this.editedItem._id}`) 
            }
