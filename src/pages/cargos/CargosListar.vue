<template>
    <v-row>
        ...
        <v-col cols="12" class="py-0">
            <v-data-table
                no-data-text="Cargos no disponibles"
                ...
                :server-items-length="totalCargos"
                :loading="loading"
                :search="search"
                class="elevation-1"
            >
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <router-link :to="{ name: 'CargoDetalle', params: { id: item._id, action:'editar'}}" >
                        <v-icon
                            small
                            class="mr-2"
    ...
    methods: {
        ...
        async readDataFromAPI() {
            ...
            if (userLogged) {
                ...
                let response = await api.call_respuesta_directa('/cargos') 
                cargos = response.data ? response.data.data : [];
                this.loading = false;
                this.totalCargos = response.data.totalCargos;
        ...
        async deleteApi() {
            ...
            if (userLogged) {
                ...
                response = await api.call_respuesta_directa(`/cargos/${this.editedItem._id}`) 
            }
            if (response && response.status == 200) {
                this.cargos.splice(this.editedIndex, 1)
                this.closeDelete()
                this.readDataFromAPI()
            }else{
                this.$notify(
                    {
                        group: "bottom",
                        title: "Error",
                        text: "No se pudo eliminar cargo",
                        type: "error"
                    },
                    4000
                );
