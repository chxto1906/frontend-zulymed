<template>
    <v-row>
        ...
        <v-col cols="12" class="py-0">
            <v-data-table
                no-data-text="Departamentos no disponibles"
                ...
                :server-items-length="totalDepartamentos"
                :loading="loading"
                :search="search"
                class="elevation-1"
            >
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <router-link :to="{ name: 'DepartamentoDetalle', params: { id: item._id, action:'editar'}}" >
                        <v-icon
                            small
                            class="mr-2"
    ...
    methods: {
        ...
        async readDataFromAPI() {
            ...
            if (userLogged) {
                ...
                let response = await api.call_respuesta_directa('/departamentos') 
                departamentos = response.data ? response.data.data : [];
                this.loading = false;
                this.totalDepartamentos = response.data.totalDepartamentos;
        ...
        async deleteApi() {
            ...
            if (userLogged) {
                ...
                response = await api.call_respuesta_directa(`/departamentos/${this.editedItem._id}`) 
            }
            if (response && response.status == 200) {
                this.departamentos.splice(this.editedIndex, 1)
                this.closeDelete()
                this.readDataFromAPI()
            }else{
                this.$notify(
                    {
                        group: "bottom",
                        title: "Error",
                        text: "No se pudo eliminar departamento",
                        type: "error"
                    },
                    4000
                );
